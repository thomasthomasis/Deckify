...
export default function StudyPage() {
  ...
  async function loadCards() {
    ...
    const dueCards = data?.filter((review) => review.cards?.deck_id === deckId) ?? [];

    setCards(dueCards.map((review) => review.cards));
  }
    ...
    return (
      <main className="mx-auto max-w-3xl p-8">
        <div className="flex min-h-[70vh] flex-col items-center justify-center text-center">
          ...
          <h2 className="mt-6 text-3xl font-bold">You're done for today</h2>
  ...
  return (
    <main className="mx-auto max-w-4xl p-8">
      <div className="mb-8">
        <p className="text-zinc-400">Deckify Learning Session</p>
      ...
      <StudyCard content={showAnswer ? card.back : card.front} showAnswer={showAnswer} />
      ...
      {!showAnswer ? (
        ...
        <div className="mt-8 grid grid-cols-2 gap-4 md:grid-cols-4">
          <Button variant="destructive" onClick={() => submitReview('again')}>
            Again
          </Button>

          <Button variant="secondary" onClick={() => submitReview('hard')}>
            Hard
          </Button>

          <Button onClick={() => submitReview('good')}>Good</Button>

          <Button className="bg-green-500 hover:bg-green-400" onClick={() => submitReview('easy')}>
            Easy
          </Button>
        </div>
      )}
